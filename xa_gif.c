...
static xaLONG gif_anim_mask;  /* from GIF89 f9 extension */
  ...
  while(!exit_flag)
  {
    ...
    if (gif_anim_type == 0x01)
    {
      ACT_Setup_Mapped(act, pic_ptr, gif_chdr_now,
			...
			gif_screenx,gif_screeny,xaTRUE,gif_anim_mask,
    ...
    else
    {
       ACT_Setup_Mapped(act, pic_ptr, gif_chdr_now,
			gifimage.left,gifimage.top,gif_imagex,gif_imagey,
			gif_screenx,gif_screeny,xaFALSE,0,xaTRUE,xaFALSE,xaFALSE);
    }
...
/*CODE*/
xaULONG GIF_Get_Code(fp) /* get code depending of current LZW code size */
FILE *fp;
{
  xaULONG code;
  register xaLONG tmp;
...
char *filename;
{
 ...
 firstword  = (xaULONG)(fgetc(fp) & 0xff) << 24;
 firstword |= (xaULONG)(fgetc(fp) & 0xff) << 16;
 firstword |= (xaULONG)(fgetc(fp) & 0xff) <<  8;
 firstword |= (xaULONG)(fgetc(fp) & 0xff);

 fclose(fp);

 if (firstword == 0x47494638) return((xaLONG)xaTRUE);
 return((xaLONG)xaFALSE);
}
...
FILE *fp;
{
  ...
  while(block_size > 0)
  {
    ...
    {
      tmp=fgetc(fp);
      if ( (code == 0xfe) && (xa_verbose==xaTRUE) ) fprintf(stderr,"%c",tmp);
