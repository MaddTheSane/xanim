...
static ULONG set_back_num,set_sset_num;
static SET_FRAM_HDR *set_frames;
static ULONG set_fram_num;
static SET_SSET_HDR *set_sset_start,*set_sset_cur;
static XA_ACTION *work_act;
static XA_ACTION *back_act;
static ULONG set_time;
static LONG set_xscroll_flag,set_xscroll_len,set_yscroll_flag,set_yscroll_len;
static LONG set_back_xpos,set_back_ypos;
static ULONG set_back_scroll_fram;
static LONG set_sset_cur_num,set_sset_xoff,set_sset_yoff;
static ULONG set_multi_flag;
static ULONG set_or_mask;

static char set_buff[512];
static char set_buff1[64];
static Bit_Map_Header bmhd;

static ULONG set_imagex,set_imagey,set_imagec,set_imaged;
...
XA_ANIM_HDR *anim_hdr;
{
  ...
  {
    UBYTE *t_pic;
    t_pic = (UBYTE *)malloc( XA_PIC_SIZE(psize) );
	...
	set_imagex,set_imagey,FALSE,0,TRUE,TRUE,FALSE);
    work_act = act;
  }
...
char *filename;
{
  ...
  if ( (d1 == 0x78536365) && (d2 == 0x6e654564) )return(TRUE);
  /* check for "xMovieSe"tter */
  if ( (d1 == 0x784d6f76) && (d2 == 0x69655365) )return(TRUE);
  return(FALSE);
}

ULONG SET_Read_File(fname,anim_hdr)
char *fname;
XA_ANIM_HDR *anim_hdr;
{
  FILE *fin;
  ...
  while(!feof(fin) && !exit_flag)
  {
   XA_ACTION *act;
     ...
     case 'a':  /* 256 ascii 32 ascii IFF stuff  Background Image */
	{
	  ...
	  if (work_act == 0)
	  {
		...
		0,0,tmp_back_hdr->xsize,tmp_back_hdr->ysize,
	        set_imagex,set_imagey,FALSE,0,TRUE,FALSE,FALSE);
     ...
     case 'b':  /* 256 ascii 32 ascii IFF stuff  Faces */
	{
	  ...
	  set_buff1[32] = 0;
	  DEBUG_LEVEL1 fprintf(stderr,"   %s\n",set_buff1);
          if (set_multi_flag == FALSE) SET_Read_SSET(anim_hdr,fin,tmp_sset_hdr);
          else
          {
     ...
     case 'e': /* position face 12 bytes */
	{
	  ...
	  ULONG face_num,depth,garb,fram_num;
	  LONG xpos,ypos,xoff,yoff;
	  LONG back_x,back_y;
...
DEBUG_LEVEL2 fprintf(stderr,"      e FACE off <%ld,%ld> fin <%ld,%ld>\n",
	...
	fread((char *)set_buff1,1,32,fin); set_buff1[32] = 0;
	DEBUG_LEVEL1 fprintf(stderr,"   %s\n",set_buff1);
        if (set_multi_flag == FALSE) SET_Read_Sound_IFF(fin);
	/* else ignore */
	break;
...

 {
   ...
   XA_ACTION *back_act;
   ULONG fram_time;
   ULONG buffx,buffy,need_to_scale;
   LONG t_time;
...
UBYTE *SET_Read_BACK(fin,back_hdr)
FILE *fin;
SET_BACK_HDR *back_hdr;
{
  ...
  while(form_size)
  {
      ...
      case BODY:
	{
	  ...
	  xsize = bmhd.width;
	  ysize = bmhd.height;
	  pic = (UBYTE *) malloc( XA_PIC_SIZE(xsize * ysize) );
	  if (pic == 0) SET_TheEnd1("SET_Read_BODY: malloc failed\n");
	  IFF_Read_BODY(fin,pic,size, xsize, ysize, (ULONG)(bmhd.depth),
...
SET_SSET_HDR *set_hdr;
{
  ...
  for(i = 1; i < list_num; i++)
  {
	...
	set_imagex,set_imagey,TRUE,set_or_mask,TRUE,FALSE,FALSE);
    ACT_Add_CHDR_To_Action(act,set_chdr);
    face->face_act = act;
  }
}
...
UBYTE *SET_Read_FACE(fin,face_hdr)
FILE *fin;
SET_FACE_HDR *face_hdr;
{
  ULONG chid,data;
  ...
  fprintf(stderr,"      FACE size <%ld,%ld> p <%ld,%ld> off <%ld,%ld>\n",
	  (LONG)face_hdr->xsize,(LONG)face_hdr->ysize,
	  (LONG)face_hdr->x,    (LONG)face_hdr->y,
	  (LONG)face_hdr->xoff, (LONG)face_hdr->yoff );
	}
	break;
      case BODY:
	{
	  ...
	  IFF_Read_BODY(fin,pic,size,
		...
		(ULONG)(bmhd.compression), (ULONG)(bmhd.masking),set_or_mask);
	  form_size = 0; /* body is always last */
	  break;
	}
      default: 
