...
extern LONG x11_error_possible;
extern ULONG xa_speed_change;
extern LONG xa_remote_ready;
LONG xa_remote_realized = FALSE;
LONG xa_remote_busy = FALSE;
...
int            *np;
{
  if (xa_remote_ready != TRUE) return;
  if (xa_remote_busy == TRUE) return;
#ifdef XA_REMOTE_CONTROL
  if (xa_remote_realized == FALSE)
  {
    xa_remote_busy = TRUE;
    XA_Realize_Remote(remote_widget,last_widget);
    xa_remote_busy = FALSE;
  }
  else
  {
    xa_remote_busy = TRUE;
...
{
  xa_audio_volume -= 5; if (xa_audio_volume < 0) xa_audio_volume = 0;
  xa_audio_newvol = TRUE;
}
/* decrement xa_audio_volume by 1 */
ACTION_PROC(xanim_dec_audio_1)
{
  xa_audio_volume -= 1; if (xa_audio_volume < 0) xa_audio_volume = 0;
  xa_audio_newvol = TRUE;
}
...
{ int button = (xa_audio_mute==TRUE)?(BM_VOFF):(BM_VON);
  xa_audio_mute = (xa_audio_mute==TRUE)?(FALSE):(TRUE);
  xa_audio_newvol = TRUE;
#ifdef XA_REMOTE_CONTROL
...
/* Query Black and White */
  ...
  { XA_CHDR *chdr = xa_chdr_first;
    backgnd = CMAP_Find_Closest(chdr->cmap,chdr->csize,0,0,0,8,8,8,TRUE);
    foregnd = CMAP_Find_Closest(chdr->cmap,chdr->csize,255,255,255,8,8,8,TRUE);
  }
  else
  {
    foregnd = WhitePixel(theDisp,theScreen);
 ...
 {
   BM_pmap[i] = XCreatePixmapFromBitmapData(theDisp,mainW,BM_bits[i],
   ...
   * xa_remote_ready is set to TRUE in the xa_remote_expose routine.
   */
  XtAddRawEventHandler(last_widget, ExposureMask, FALSE, xa_remote_expose, NULL);
  /******** REALIZE THE WIDGET *******/
  if (remote_flag == TRUE) XA_Realize_Remote(remote_widget,last_widget);
  else xa_remote_ready = TRUE;
}

void XA_Realize_Remote(remote,last)
Widget remote,last;
{
  xa_remote_realized = TRUE;
